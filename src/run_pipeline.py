"""
End-to-end training + CV + submission script.
Usage:
...
def build_pipeline_for_columns(X, text_col='mentor_feedback_text'):
    ...
    if text_features:
        # ColumnTransformer expects the name of the column for text
...
def run_cv_and_submit(train_path, test_path, out_submission, model_out, cv=5, random_state=42):
    mlflow.set_experiment("Datathon2026_LightGBM")
    mlflow.start_run()
    ...
    pipeline = build_pipeline_for_columns(pd.concat([X, X_test], axis=0))
